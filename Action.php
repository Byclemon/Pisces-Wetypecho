...
class WeTypecho_Action extends Typecho_Widget implements Widget_Interface_Do {
    ...
    private function addcomment()
    {
        ...
        $coid =$this->db->query($this->db->insert('table.comments')->rows(array('cid' => $cid, 'created' => time(), 'author' => $author, 'authorId' => '0',
                'ownerId' => '1', 'mail' => '2280940837@qq.com', 'url' => 'https://www.anhaowu.com', 'ip' =>'8.8.8.8', 'agent' => 'miniprogram', 'text' => $text, 'type' => 'comment',
            ...
            if(sizeof($cids)>0){
                foreach($cids as $cid) {
                    ...
                    if(sizeof($post)>0 && $post[0]!=null) {
                        ...
                        $post[0]['thumb'] =  $this->db->fetchAll($this->db->select('str_value')->from('table.fields')->where('cid = ?', $post['cid'])->where('name = ? ',thumb))?$this->db->fetchAll($this->db->select('str_value')->from('table.fields')->where('cid = ?', $post['cid'])->where('name = ? ',thumb)):array(array("str_value"=>"https://tuku.ycygame.cn/anhaowu/shoutu.png"));
